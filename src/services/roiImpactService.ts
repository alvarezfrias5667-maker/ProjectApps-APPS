...
export function calculateRoiImpact(input: RoiInput): RoiResult {
  ...
  let shortExplanation = "El activo ofrece un retorno financiero sólido sustentado en optimización interna de procesos.";
  if (input.opportunityType === "Enterprise White Label") {
    shortExplanation = "Alta rentabilidad apalancada en licenciamiento externo con gastos mínimos de mantenimiento.";
  } else if (roiScore > 6.0) {
    shortExplanation = "Sinergia brillante de alto volumen de horas ahorradas y excelente oportunidad de rentabilización externa.";
  } else if (input.complexity === "High" && roiScore < 2.0) {
    shortExplanation = "Complejidad de construcción elevada que mitiga el retorno inicial. Considere simplificar la arquitectura.";
  }
